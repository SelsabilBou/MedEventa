...
const createWorkshopController = (req, res) => {
  ...
  if (!mysqlDate) return res.status(400).json({ message: 'Date invalide' });

  eventExists(eventId, (err, exists) => {
    if (err) return res.status(500).json({ message: 'Erreur serveur', error: err.message });
    if (!exists) return res.status(404).json({ message: 'Événement introuvable' });

    userExists(responsable_id, (err2, userOk) => {
      if (err2) return res.status(500).json({ message: 'Erreur serveur', error: err2.message });
      if (!userOk) return res.status(404).json({ message: 'Responsable introuvable' });
  ...
  getWorkshopById(workshopId, (err, workshop) => {
    ...
    userExists(merged.responsable_id, (err2, userOk) => {
      ...
      updateWorkshop(workshopId, merged, (err3, affectedRows) => {
        if (err3) return res.status(500).json({ message: 'Erreur serveur', error: err3.message });
        if (!affectedRows) return res.status(400).json({ message: 'Aucune modification' });
